...
@rating_bp.route('/rating/<int:user_rated>', methods=['POST'])
def create_rating(user_id):
    data = request.get_json()

    return handle_rating_process(data,user_id)


@rating_bp.route('/rating/<int:user_rated>/user/', methods=['PATCH'])
def update_rating(user_id,user_rated):
    data = request.get_json()
    return handle_rating_process(data, user_id ,user_rated)

def handle_rating_process(data, user_id ,user_rated=None):
    rating = Rating()
